...
class DeviceEnergyManagementClusterSlotStruct(
  ...
  val nominalPower: Optional<Int>,
  val minPower: Optional<Int>,
  val maxPower: Optional<Int>,
  val nominalEnergy: Optional<Int>,
  val costs: Optional<List<DeviceEnergyManagementClusterCostStruct>>,
  val minPowerAdjustment: Optional<Int>,
  val maxPowerAdjustment: Optional<Int>,
  ...
  companion object {
    ...
    private const val TAG_MANUFACTURER_E_S_A_STATE = 6
    private const val TAG_NOMINAL_POWER = 7
    private const val TAG_MIN_POWER = 8
    private const val TAG_MAX_POWER = 9
    private const val TAG_NOMINAL_ENERGY = 10
    private const val TAG_COSTS = 11
    private const val TAG_MIN_POWER_ADJUSTMENT = 12
    private const val TAG_MAX_POWER_ADJUSTMENT = 13
    private const val TAG_MIN_DURATION_ADJUSTMENT = 14
    private const val TAG_MAX_DURATION_ADJUSTMENT = 15
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader): DeviceEnergyManagementClusterSlotStruct {
      ...
      val nominalPower =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_NOMINAL_POWER))) {
          Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_NOMINAL_POWER)))
        } else {
          Optional.empty()
        }
      val minPower =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MIN_POWER))) {
          Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_MIN_POWER)))
        } else {
          Optional.empty()
        }
      val maxPower =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MAX_POWER))) {
          Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_MAX_POWER)))
        } else {
          Optional.empty()
        }
      val nominalEnergy =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_NOMINAL_ENERGY))) {
          Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_NOMINAL_ENERGY)))
        } else {
          Optional.empty()
        }
      val costs =
      ...
      val minPowerAdjustment =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MIN_POWER_ADJUSTMENT))) {
          Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_MIN_POWER_ADJUSTMENT)))
        } else {
          Optional.empty()
        }
      val maxPowerAdjustment =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MAX_POWER_ADJUSTMENT))) {
          Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_MAX_POWER_ADJUSTMENT)))
        } else {
          Optional.empty()
        }
